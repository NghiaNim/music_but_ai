    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: isDark ? "#111111" : "#FFFAEF" }}>
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
        {isPending ? (
          ...
          <View style={{ borderRadius: 14, backgroundColor: "#FEF2F2", padding: 14 }}>
          ...
          <View style={{ gap: 10 }}>
            {orders.map((order) => {
              const statusColors =
                STATUS_COLORS[order.status] ?? { bg: "#FEF3C7", text: "#B45309" };
              return (
                <Pressable
                  key={order.id}
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={{ color: "#6B7280", fontSize: 12, marginTop: 2 }}>
                      {formatMonthDay(new Date(order.event.date))} · {order.quantity}x $
